 ...
 */
@ManagedBean(name = "rotacionOperario")
@ViewScoped
public class RotacionesOperarioBean implements Serializable{
	...
	private Collection<PosicionRotacionJPA> rotacionOperarioLista;
	private int screen = 0;
	protected Collection<PosicionRotacionJPA> rotacionOperarioListaVista;
	protected int numPosicion = 0;
	private int id;
	private String operarioId="";
	private EncargadoJPA operario;
   ...
   // private int[][] asignacion= new int[numPuestos][maxPuesto];
	 ...
	 * @throws Exception
	 */
	public RotacionesOperarioBean() throws Exception
	{
		String a= "Ini";
		try {
	...
	{
		int n =0;
		rotacionOperarioListaVista = new ArrayList<PosicionRotacionJPA>();
		for (Iterator<PosicionRotacionJPA> iter2 = rotacionOperarioLista.iterator(); iter2.hasNext();)
		{
			PosicionRotacionJPA cap = (PosicionRotacionJPA) iter2.next();
			if (n >= screen*15 && n< (screen*15+15))
			{				
				this.rotacionOperarioListaVista.add(cap);
			}
			n +=1;
		}
		this.numPosicion = n;
		return rotacionOperarioListaVista;
	}
	...
	{
		if (((screen+1)*15 < rotacionOperarioLista.size()))
		{
			screen +=1;
		}
	}
	...
	/**
	 * Method used for Facelet to call rotacionOperarioListaVista Facelet
	 * @return Facelet name
	 * @throws Exception
	 */
	public String listaOperarioRotacion() throws Exception
	{
		rotacionOperarioLista();
		return "rotacionOperarioListaVista";
	}
	...
	public void setEncargadoId(String operarioId) {
		try {
		int id= Integer.valueOf(operarioId);
		Properties props = System.getProperties();
		Context ctx = new InitialContext(props);
	...
	public void setEncargado(EncargadoJPA operario) {
